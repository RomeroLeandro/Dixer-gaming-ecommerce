...
export const ItemCart = ({item}) => {
  ...
  return (
    <div className={`itemCart ${darkMode && "itemCartDark"}`}>
      <Link className='imgCart' to={`/item/${item.id}`}>
        <img src={item.img} alt={`Imagen de ${item.nombre}`} className='img-fluid rounded-start'/>
      </Link>
      <Link className={`nameCart ${darkMode && "nameCartDark"}`} to={`/item/${item.id}`}>
        <h5>{item.nombre} {item.marca} {item.modelo}</h5>
      </Link>
      <p className={`cantCart ${darkMode && "cantCartDark"}`}>Cantidad: {new Intl.NumberFormat('de-DE').format(item.cant)}</p>
      <div className={`priceItemCart ${darkMode && "priceItemCartDark"}`}>
      ...
      <div className='delete'>
        <button className={`deleteButton ${darkMode && "deleteButtonDark"}`} onClick={() => removeItem(item.id)}>
